...
extern "C" {
	...
	// Read byte from port
	[[nodiscard]]
	auto	outPort8(const igros::i386::port_t addr) noexcept -> igros::igros_byte_t;
	// Read word from port
	[[nodiscard]]
	auto	outPort16(const igros::i386::port_t addr) noexcept -> igros::igros_word_t;
	// Read long from port
	[[nodiscard]]
	auto	outPort32(const igros::i386::port_t addr) noexcept -> igros::igros_dword_t;

	// Write byte to port
	void	inPort8(const igros::i386::port_t addr, const igros::igros_byte_t value) noexcept;
	// Write word to port
	void	inPort16(const igros::i386::port_t addr, const igros::igros_word_t value) noexcept;
	// Write long to port
	void	inPort32(const igros::i386::port_t addr, const igros::igros_dword_t value) noexcept;
